...
class ServerWebInputMappingTest {

  private ServerWebInputMapping serverWebInputMapping;

  @BeforeEach
  void beforeEach() {
    serverWebInputMapping = new ServerWebInputMapping(new IdentityProblemFormat());
  }
  ...
  void givenExceptionWithCauseAndWithoutPropertyName_shouldDelegateAndIncludeMethodParameter()
    ...
    Problem problem = serverWebInputMapping.map(ex, new HttpHeaders(), HttpStatusCode.valueOf(400));
  ...
  void givenExceptionWithoutCause_shouldReturnSimpleProblem() {
    ...
    Problem problem = serverWebInputMapping.map(ex, null, null);
